...
export default function PatientSearch({ onScanClick, onSavePatient }) {
  ...
  // handleScanQR removed from here since it's now handled by topbar exclusively
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Map backend schema to expected frontend model
        setPatientData({
          id: p.id || p.abhaId,
          abhaId: p.abhaId,
          name: p.name,
          ...
          // Assuming these are missing from basic mock_abha_users table, we add placeholders 
          // or you can load them from highly specific relational tables.
          ...
          <div className="relative w-full rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/20 mb-8" style={{ minHeight: '340px' }}>
             ...
             <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
                ...
                <form onSubmit={handleSearch} className="relative group w-full max-w-2xl px-2">
                  ...
                  <input
                    type="text"
                    className="block w-full pl-14 pr-40 py-5 border-4 border-white/25 rounded-2xl leading-5 bg-white/95 backdrop-blur-md placeholder-slate-400 focus:outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/30 sm:text-lg transition-all shadow-2xl text-slate-800 font-medium"
