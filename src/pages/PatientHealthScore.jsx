  ...
  if (!fullBodyReport) {
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 flex flex-col items-center justify-center text-center mt-12">
        ...
        <h2 className="text-3xl font-black text-slate-800">Unlock your Health Score</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800">Health Benchmarks</h1>
          <p className="text-slate-500 text-sm mt-1">Based on Full Body Checkup ({fullBodyReport.date})</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          <h3 className="font-bold text-slate-800 flex items-center gap-2 text-lg border-b border-slate-100 pb-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-slate-700">Optimal: Inside benchmark bounds</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 rounded-full bg-amber-500" />
              <span className="text-sm font-medium text-slate-700">Borderline: Monitor via diet & habits</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 rounded-full bg-red-500" />
              <span className="text-sm font-medium text-slate-700">High Risk: High priority medical check</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 mt-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Tracking your 3-year historical uploads.</p>
          </div>
          <div className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-full text-sm font-bold flex items-center justify-center gap-2 w-max shadow-sm border border-emerald-100">
        ...
        <div className="h-56 flex items-end justify-between gap-4 px-2 sm:px-12 relative w-full pt-6">
          ...
          <div className="absolute inset-0 flex flex-col justify-between pt-6 pb-12 z-0">
             <div className="w-full border-t border-dashed border-slate-200 relative"><span className="absolute -top-3 -left-6 text-[10px] text-slate-400 font-bold">140</span></div>
             <div className="w-full border-t border-dashed border-slate-200 relative"><span className="absolute -top-3 -left-6 text-[10px] text-slate-400 font-bold">100</span></div>
             <div className="w-full border-t border-dashed border-slate-200 relative"><span className="absolute -top-3 -left-6 text-[10px] text-slate-400 font-bold">60</span></div>
          </div>
            ...
            return (
              <div key={trend.year} className="flex flex-col items-center gap-3 relative z-10 w-full group">
                ...
                <div className="text-center absolute -bottom-12">
                  <span className="block text-sm font-black text-slate-800">{trend.year}</span>
                  <span className="text-[10px] uppercase font-bold text-slate-400 whitespace-nowrap">{trend.label}</span>
                </div>
              </div>
            );
          })}
      ...
      <h3 className="text-xl font-bold text-slate-800 pt-8 border-t border-slate-200">Biometric Report Breakdown</h3>
      <div className="grid grid-cols-1 gap-4">
        {Object.entries(fullBodyReport.metrics).map(([key, data]) => (
          <div key={key} className="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:border-indigo-100 hover:shadow-md">
            
            <div className="flex items-center gap-5 md:w-1/3">
              <div className="w-12 h-12 bg-indigo-50 text-indigo-500 rounded-xl flex items-center justify-center shrink-0">
                {getMetricIcon(key)}
              </div>
              <h4 className="font-bold text-slate-800 whitespace-nowrap">{formatMetricName(key)}</h4>
            </div>

            <div className="flex-1 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Your Value:</span>
